...
const LOADING_MESSAGES = [
    "팀원들의 경력 정보가 부족한 점을 반영해 추가적인 학습 기간을 확보했어요.",
    "구체적인 프로젝트 명세가 없어 요구사항 분석 단계를 추가했어요.",
    "팀원들의 직군을 반영해 프론트엔드, 백엔드, 디자이너 각 직군별 일정을 생성했어요.",
    "마감기한이 짧으므로 상세한 테스트 단계는 생략했어요.",
    "뤼이도 AI를 통해 효율적으로 프로젝트를 일정을 생성하세요!",
];
...
const PARTICIPANTS = [
    ...
    { id: 3, name: "박진홍", roleType: "member" as const, included: true },
    { id: 4, name: "강동학", roleType: "member" as const, included: true },
    { id: 5, name: "최정윤", roleType: "member" as const, included: true },
    { id: 6, name: "전중환", roleType: "member" as const, included: true }
];
            ...
            <TableCell style={{ width: "300px" }}>
                {isEditingName ? (
                    <input
                        ...
                        autoFocus
                        className="w-full rounded-md border border-brand bg-white px-2 py-1 text-base text-primary outline-none transition"
                    />
                ) : (
                    <button
                        onClick={() => setIsEditingName(true)}
                        className="w-full text-left rounded-md border border-primary/20 cursor-pointer bg-white px-2 py-1 text-base text-primary outline-none transition"
                    >
                        {name}
                    </button>
                )}
...
function MainPage() {
    ...
    const handleBackToLoading = () => {
        ...
        setCurrentStepIndex(loadingIndex === -1 ? 4 : loadingIndex);
    };
    ...
    return (
        <div className="min-h-screen bg-white text-primary relative overflow-hidden">
            ...
            />
            <div className="absolute cursor-pointer transform top-1/4 left-0 -translate-y-[108px] w-[165px] h-[36px] border-2 border-brand shadow-[0_0_8px_6px_rgba(93,79,249,0.15)]"
                onClick={() => setIsModalOpen(true)}
            />
            <div className="absolute transform top-1/4 left-1/5 -translate-x-[120px] -translate-y-[108px] w-[240px] bg-tertiary border border-primary/20 rounded-sm p-3">
                <p className="text-sm font-semibold text-black">
                    Riido AI 기능을 사용해보세요!
                </p>
                <p className="text-sm text-primary">
                    초보 PM이나 관리자가 없는 팀도 간단한 입력만으로 즉시 실행 가능한 일정과 작업을 자동으로 짤 수 있어요.
                </p>
            </div> */}
            {isModalOpen && (
                <>
                                ...
                                <div className="flex-1 overflow-auto py-4 px-6">
                                                ...
                                                <Table className="w-full scrollbar-hide">
                                                    <TableHeader>
                                                        <TableRow>
                                                            ...
                                                            <TableHead style={{ width: "220px" }}>직군/담당 <span className="text-sm text-primary opacity-80">(선택)</span></TableHead>
                                                            <TableHead style={{ width: "220px" }}>경력 <span className="text-sm text-primary opacity-80">(선택)</span></TableHead>
                                            ...
                                            <div className="w-[400px]">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    {visibleMessages.map((messageIndex) => {
                                                        ...
                                                        const opacity = position === 0 ? 1 : position === 1 ? 0.6 : position === 2 ? 0.4 : position === 3 ? 0.2 : 0.1;
                                    ...
                                    {currentStep.id === "board" ? (
                                        <div className="flex flex-col">
                                            <div className="mb-3">
                                                <h2 className="text-base font-semibold text-primary">
                                                    {LOADING_TIME / 1000}초만에 프로젝트 일정이 만들어졌어요!<br />일정을 간단하게 수정하거나, 일부만 다시 생성할 수 있어요(상세한 수정은 일정 추가 후 작업 페이지에서 가능해요).
                                                </h2>
                                            </div>
                                            <div className="relative max-h-[290px] w-full overflow-auto rounded-lg border border-sec bg-white">
                                                <Table className="w-full scrollbar-hide">
                                                    <TableHeader>
                                                        <TableRow>
                                        ...
                                        <>
                                            <div>
                                                {currentStep.id === "board" ? (
                                                    <button
                                                        type="button"
                                                        onClick={handleBackToLoading}
                                                        className="rounded-lg bg-tertiary px-5 py-2.5 font-semibold text-primary text-base transition cursor-pointer"
                                                    >
                                                        {boardCheckedCount == BOARD_ITEMS_TEMPLATE.length ? "전체 프로젝트 보드 다시 생성" : `${boardCheckedCount}개 프로젝트 보드 다시 생성`}
                                                    </button>
                                                ) : canGoBack ? (
                                                    <button
                                                        type="button"
